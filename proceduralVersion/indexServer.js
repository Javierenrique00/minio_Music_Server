exports.moduleIndex = function(bucket,pathMusic,indexFileName,minioClient){
  ...
  function musicCommonMetadata(inData,nombre,fileSize,extension){

    myObj = {"path":nombre,"filesize":fileSize,"extension":extension}
  ...
  function buscaDatosMinio(){
    ...
    if(dirList.length > 0){
      ...
      loadObjectMinio(obj.path,obj.size,obj.extension)
    }
  }


  //--- Creamos una función que carga una archivo de minio
  function loadObjectMinio(nombre,tamano,extension){
    let size = 0
    ...
    minioClient.getObject(bucket, nombre, function(err, dataStream) {
      ...
      dataStream.on('end', function() {
        ...
        mm.parseBuffer(Buffer.concat(buffer), 'audio/mpeg', { fileSize: size }).then( metadata => {
            ...
            let temDatos = musicCommonMetadata(metadata,nombre,size,extension)
            ...
            buscaDatosMinio() //--- se va a sacar a un stream de que cuando termina pide el próximo, solo uno a la vez.
        });
      })
      dataStream.on('error', function(err) {
        console.log(err)
  ...
  stream.on('data', function(obj) {
      ...
      if(nombre!=undefined){
        ...
        if(size!=undefined && extension){
          ...
          dirList.push({"path":nombre,"size":size,"extension":myExtension})
          contList++
        }
      }
